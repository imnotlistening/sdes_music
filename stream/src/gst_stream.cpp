...
int music_make_pipeline(struct music_rtp_pipeline *pipe, char *id){
	...
	GstBus *bus;
	GstElement *pipeline;
	GstElement *uridecoder, *audioconv, *volume, *audiosink, *audioresample;
	...
	gst_element_link_many(audioconv, volume, audioresample, audiosink, NULL);
...
static gboolean bus_callb(GstBus *bus, GstMessage *msg, gpointer data){
	...
	gchar *debug;
	GError *error;
    music_rtp_pipeline *pipe = (music_rtp_pipeline*)data;
